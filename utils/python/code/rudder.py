...
class RudderInventory(object):
    def __init__(self):
        ...
        # Create connection
        self.conn = http.Http(
            disable_ssl_certificate_validation=self.disable_ssl_validation
        )
    ...
    def read_settings(self):
        ...
        self.token = os.environ.get(
            "RUDDER_API_TOKEN", config.get("rudder", "token")
        )
        self.version = os.environ.get(
            "RUDDER_API_VERSION", config.get("rudder", "version")
        )
        self.uri = os.environ.get(
            "RUDDER_API_URI", config.get("rudder", "uri")
        )
    ...
    def parse_cli_args(self):
        ...
        parser = argparse.ArgumentParser(
            description="Produce an Ansible Inventory file based on Rudder inventory"
        )
        parser.add_argument(
            "--list",
            action="store_true",
    ...
    def load_cache(self):
        ...
        try:
            ...
            self.fail_with_error(
                "Could not parse JSON response from local cache",
                "parsing local cache",
            )
    ...
    def is_endpoint_defined(self, endpoint_name):
        ...
        if "endpoints" in result["data"]:
            defined = any(
                endpoint_name in x for x in result["data"]["endpoints"]
            )
        return defined
    ...
    def get_nodes(self):
        ...
        inherited_endpoint = self.is_endpoint_defined(
        ...
        for node in result["data"]["nodes"]:
            ...
            if inherited_endpoint:
                nodes[node["id"]][
                    "properties"
                ] = self.get_inherited_properties(node["id"])
            else:
        ...
        for group in groups:
            ...
            if self.fail_if_name_collision:
                if groups[group]["name"] in inventory:
                    self.fail_with_error(
                        'Name collision on groups: "%s" appears twice'
                        % groups[group]["name"],
                        "creating groups",
                    )
            ...
            for node in groups[group]["hosts"]:
                # Add node to group
                inventory[groups[group]["name"]]["hosts"].append(
                    nodes[node]["hostname"]
                )
        ...
        for node in nodes:
            ...
            if self.fail_if_name_collision:
                if nodes[node]["hostname"] in properties:
                    self.fail_with_error(
                        'Name collision on hosts: "%s" appears twice'
                        % nodes[node]["hostname"],
                        "creating hosts",
                    )
            ...
            for node_property in nodes[node]["properties"]:
                properties[nodes[node]["hostname"]][
                    self.to_safe(node_property["name"])
                ] = node_property["value"]
    ...
    def api_call(self, path):
        ...
        try:
            response, content = self.conn.request(
                target.geturl(), method, body, headers
            )
        ...
        try:
            ...
            self.fail_with_error(
                "Could not parse JSON response from Rudder API",
